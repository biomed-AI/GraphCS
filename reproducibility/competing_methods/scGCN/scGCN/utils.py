...
def save_graph_data(matrix, data_name="graph"):
    n_count=matrix.shape[0]
    f = open("./"+data_name+"_ccamnn.txt", 'w')
    f.write('{}\n'.format(n_count))
    adj = defaultdict(list)  # default value of int is 0
    for i, row in enumerate(matrix):
        for j, adjacent in enumerate(row):
...
def load_data(datadir, data_name, save_graph, origin_graph=True):
    res=input_data(DataDir=datadir, data_name=data_name)
    datadir=datadir+"/"+data_name

    PIK = "{}/datasets.dat".format(datadir)
    with open(PIK, "rb") as f:
         objects = pkl.load(f)
    data_train1, data_test1, data_val1, label_train1, label_test1, label_val1, lab_data2, lab_label2, types,total_cells = tuple(
        objects)
    ...
    features, sp_features = preprocess_features(features)
    ...
    id_gp1 = np.array([
        ...
        for i in range(len(id_graph1)) if len(np.where(find1 == id_graph1.iloc[i, 1])[0])== len(np.where(index1 == id_graph1.iloc[i, 0])[0])    
     ])

    id_gp2 = np.array([
        np.concatenate((np.where(index1 == id_graph1.iloc[i, 0])[0],
                        np.where(find1 == id_graph1.iloc[i, 1])[0]))
        for i in range(len(id_graph1)) if len(np.where(index1 == id_graph1.iloc[i, 0]) [0])==len(np.where(find1 == id_graph1.iloc[i, 1])[0])
    ])
