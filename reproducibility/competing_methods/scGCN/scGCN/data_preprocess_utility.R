...
save_processed_data <- function(count.list,label.list, data_name="example"){
    ...
    dir.create(paste0('input/', data_name))
    ...
    write.csv(label.list[[1]],file=paste0('input/', data_name, '/Label1.csv'),quote=F,row.names=F)
    write.csv(label.list[[2]],file=paste0('input/', data_name, '/Label2.csv'),quote=F,row.names=F)
