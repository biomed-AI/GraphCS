...
def graph_construct(data_name="example", outputdir="process_data/"):
    path0 = os.path.join(os.getcwd(), outputdir, data_name)
    ...
    data_path = "./input/" + data_name

    graph1 = pairss1[0].iloc[:, 0:2].reset_index()
    graph1.to_csv(data_path+'/integrate_graph.csv')

    #'@param graph2: intra-dataset graph
    graph2 = pairss2[0].iloc[:, 0:2].reset_index()
    graph2.to_csv(data_path+"/data2_graph.csv")

    label1 = label_list[0]
    label1.to_csv(data_path+'/Label1.csv', index=False)

    label2 = label_list[1]
    label2.to_csv(data_path+'/Label2.csv', index=False)
    print("over construct graph")
