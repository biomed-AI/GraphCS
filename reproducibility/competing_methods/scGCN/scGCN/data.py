...
#' data preperation
def input_data(DataDir='input', data_name="example"):

    graph_construct(data_name=data_name, outputdir='process_data/')
    ...
    total_cells=len(lab_label1.index.values)+len(lab_label2.index.values)

    types = np.unique(lab_label1['type']).tolist()

    # updating
    types2 = np.unique(lab_label2['type']).tolist()
    types3 = np.unique(types + types2).tolist()
    ...
    for i in range(0, len(p_data)):
        temD_train, temd_test, temL_train, teml_test = train_test_split(p_data[i], p_label[i], test_size=0.1, random_state=1)
        temd_train, temd_val, teml_train, teml_val = train_test_split(temD_train, temL_train, test_size=0.1, random_state=1)
    ...
    res = [
        ...
        label_val1, lab_data2, lab_label2, types3,total_cells
    ]
