...
GenerateGraph <- function(Dat1,Dat2,Lab1,K,check.unknown,Lab2){
    ...
    if (check.unknown){
        ...
        #cluster_names=as.character(hc)
        #cluster_index=which(cluster_names=="2")
        #print(Lab2$test_label[cluster_index])
...
#' @examples: load count.list and label.list from folder "example_data"
#' save_processed_data(count.list,label.list)
save_processed_data <- function(count.list,train_label, test_label,data_name="example",Rgraph=TRUE,check_unknown=FALSE){
    count.list <- pre_process(count_list=count.list,label_list=label.list)
    #browser()
    #' save counts data to certain path: 'input'
    dir.create('input');dir.create(paste0('input/', data_name)) 
    write.csv(t(count.list[[1]]),file=paste0('input/', data_name, '/Data1.csv'),quote=F,row.names=T)
    write.csv(t(count.list[[2]]),file=paste0('input/', data_name, '/Data2.csv'),quote=F,row.names=T)
    #browser()
    write.csv(train_label,file=paste0('input/', data_name, '/Label1.csv'),quote=F,row.names=F)
    write.csv(test_label,file=paste0('input/', data_name, '/Label2.csv'),quote=F,row.names=F) 
    print("save data over")
    ...
    if (Rgraph){
        ...
        new.lab1 <- train_label; new.lab2 <- test_label
        graphs <- suppressWarnings(GenerateGraph(Dat1=new.dat1,Dat2=new.dat2,
                                                 Lab1=new.lab1,K=5,
                                                 check.unknown=check_unknown,
					          Lab2=test_label))
        write.csv(graphs[[1]],file=paste0('input/', data_name,'/inter_graph.csv'),quote=F,row.names=T)
        write.csv(graphs[[2]],file=paste0('input/', data_name,'/intra_graph.csv'),quote=F,row.names=T)
      ...
      print(Training_Time)
        ...
        for (i in 1:N){
            ...
            if (i==1){
                write.csv(train_label,file=file.name,quote=F)
            }else{
                write.csv(test_label,file=file.name,quote=F)
            }
