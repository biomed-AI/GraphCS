...
def convert_data_graph_construction(name, edge_ratio,large_data="F"):

    data_name1="norm_data_1.csv"
    data_name2 = "norm_data_2.csv"
    data_path = "../process_data/" + name + "/norm_data/"
    if large_data=="T":
        # To save storage space, the big dataset was saved in h5 format in the pre-processing phase.
    ...
    bbknn_construct_graph(name, data_value_for_graph, batch_info, edge_ratio)
    

if __name__=="__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--name', type=str, default="example", help='Data_name.')
    parser.add_argument('--large_data', type=str, default="F", help='Saving big dataset with h5 format')
    # edge_ratio=100 represents not trimming inter-edges
    parser.add_argument('--edge_ratio', type=float, default=2, help='Parameters for controlling the number of inter-edges between batches.')
    args = parser.parse_args()

    convert_data_graph_construction(args.name,args.edge_ratio,large_data=args.large_data)
